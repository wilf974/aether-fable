"""ConvDQN training runner V3.6 — pipeline 2D + assessment + best-checkpoint."""
...
@dataclass
class ConvDQNRunnerResult:
    train_metrics: list[ConvEpisodeMetric] = field(default_factory=list)
    assessment_metrics: list[ConvAssessmentMetric] = field(default_factory=list)
...
def _run_conv_assess_episode(
    env: SoloSeasonalEnv, agent: ConvDQNAgent, seed: int
    ...
    while not (terminated or truncated):
        action = agent.act(obs2d, greedy=True)
...
def conv_assess(
    ...
    *,
    ...
    results = [
        _run_conv_assess_episode(env, agent, seed=base_seed + i)
...
def run_conv_dqn_training(
    ...
    *,
    ...
    checkpoint_path: str | Path = "checkpoints/conv_dqn_best.pt",
    patience: int = 15,
    ...
    for episode_idx in range(n_episodes):
        ...
        while not (terminated or truncated):
            action = agent.act(obs2d)
            _, reward, terminated, truncated, info = env.step(action)
            next_obs2d = env.observation_2d()
            done = terminated or truncated
            agent.observe(obs2d, action, reward, next_obs2d, done)
            obs2d = next_obs2d
            total_reward += reward
            if info.get("ate"):
                food_eaten += 1

        metric = ConvEpisodeMetric(
            episode=episode_idx,
